...
/**
 * PATCH /api/designs/:id/requirements — update a single requirement.
 */
export default defineEventHandler(async (event) => {
  ...
  const updated = { ...(design.requirements as Record<string, unknown>) }
  if (value === null) {
  ...
  const [result_] = await db().update(designs)
    .set({ requirements: updated, updatedAt: new Date() })
    .where(and(eq(designs.id, id), eq(designs.userId, userId)))
    .returning()
